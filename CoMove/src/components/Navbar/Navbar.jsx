...
function Navbar({ children }) {
  ...
  const isLoadingUser =
    !!localStorage.getItem("auth") && (!authUser.data || !authUser.data.name);
  ...
  return (
    ...
    >
      <AppShell.Header
        className={`nav_header ${scrolled ? "nav_scrolled" : ""}`}
      >
          ...
          <Stack>
            {links.map((x) => (
              <Link
                ...
                to={x.to}
                className={
                  location.pathname === x.to ? "nav_link active" : "nav_link"
                }
              >
                {x.name}
              </Link>
